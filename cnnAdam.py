...
def loop():   
    for optimizer_type in [torch.optim.Adam, torch.optim.NAdam]:
        global file_counter

        BASE_DIR = f"{optimizer_type.__name__}/{DATA_AUG}_DATA_AUG"
        ...
        with open(f'{BASE_DIR}/test_performance.csv', 'w', newline='') as csvfile:
            ...
            for lrn in (0.0001, 0.001, 0.01):
                for bsz in (16, 32, 64, 128):
                    ...
                    optimizer = optimizer_type(model.parameters(), lr=learn_rate, weight_decay=1e-5)
